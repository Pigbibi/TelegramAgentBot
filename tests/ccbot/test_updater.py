...
def test_load_update_settings_reads_env(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("CCBOT_AUTO_UPDATE", "true")
    monkeypatch.setenv("CCBOT_UPDATE_INTERVAL_SECONDS", "42")
    monkeypatch.setenv("CCBOT_UPDATE_REQUIRE_IDLE", "false")
    monkeypatch.setenv("CCBOT_UPDATE_BUSY_RETRY_SECONDS", "90")
    monkeypatch.setenv("CCBOT_UPDATE_REMOTE", "origin")
    monkeypatch.setenv("CCBOT_UPDATE_BRANCH", "main")
    monkeypatch.setenv("CCBOT_UPDATE_RUN_UV_SYNC", "false")
...
def test_load_codex_update_settings_reads_env(monkeypatch) -> None:
    monkeypatch.setenv("CCBOT_CODEX_UPDATE_CHECK", "true")
    monkeypatch.setenv("CCBOT_CODEX_AUTO_UPDATE", "true")
    monkeypatch.setenv("CCBOT_CODEX_UPDATE_PACKAGE", "@openai/codex")
    monkeypatch.setenv("CCBOT_CODEX_COMMAND", "IS_SANDBOX=1 codex --danger")
    monkeypatch.setenv("CCBOT_CODEX_UPDATE_NPM", "npm")
...
def test_dirty_worktree_skips_update(tmp_path: Path) -> None:
    (tmp_path / ".git").mkdir()
    runner = FakeRunner({("git", "status", "--porcelain"): " M src/ccbot/main.py\n"})
