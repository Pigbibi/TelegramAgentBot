...
from telegram_agent_bot.config import ProjectRoot
...
from telegram_agent_bot.backends.base import BackendInfo
...
from telegram_agent_bot.session import WindowState
...
class TestExistingWindowBinding:
    ...
    async def test_remote_bound_topic_forwards_text_without_tmux_window(self):
        ...
        with (
            ...
            patch(
                "telegram_agent_bot.bot.send_agent_message",
                new_callable=AsyncMock,
            ) as send_message,
            patch("telegram_agent_bot.bot._cancel_bash_capture") as cancel_capture,
        ):
    ...
    async def test_untracked_unbound_windows_fall_back_to_directory_browser(self):
        ...
        with (
            ...
            patch("telegram_agent_bot.bot.session_manager") as mock_sm,
            patch("telegram_agent_bot.bot.tmux_manager") as mock_tmux,
            patch("telegram_agent_bot.bot.config.project_roots_configured", False),
            patch(
                "telegram_agent_bot.bot.build_directory_browser"
    ...
    async def test_unbound_topic_shows_root_picker_before_directory_browser(self):
        ...
        with (
            ...
            patch("telegram_agent_bot.bot.session_manager") as mock_sm,
            patch("telegram_agent_bot.bot.tmux_manager") as mock_tmux,
            patch("telegram_agent_bot.bot.config") as mock_config,
            patch(
                "telegram_agent_bot.bot.safe_reply", new_callable=AsyncMock
            ) as safe_reply,
        ):
    ...
    async def test_remote_backend_roots_take_priority_in_remote_mode(self):
        ...
        with (
            patch("telegram_agent_bot.bot.agent_backend", backend),
            patch("telegram_agent_bot.bot.is_user_allowed", return_value=True),
            patch("telegram_agent_bot.bot._get_thread_id", return_value=42),
            patch("telegram_agent_bot.bot.session_manager") as mock_sm,
            patch("telegram_agent_bot.bot.tmux_manager") as mock_tmux,
    ...
    async def test_interrupt_command_interrupts_busy_codex_before_forwarding_text(self):
        ...
        with (
            ...
            patch(
                "telegram_agent_bot.bot.capture_agent_output",
                new_callable=AsyncMock,
            ...
            patch(
                "telegram_agent_bot.bot._discard_queued_agent_input",
                new_callable=AsyncMock,
                return_value=1,
            ) as discard_queue,
            patch(
            ...
            patch("telegram_agent_bot.bot.mark_window_working", new_callable=AsyncMock),
            patch("telegram_agent_bot.bot._cancel_bash_capture"),
