    ...
    env.update(
        {
            ...
            "TELEGRAM_AGENT_BOT_DIR": str(home / ".telegram-agent-bot"),
        }
    )
    return env
...
def test_linux_bootstrap_allows_checkout_outside_project_roots(
    ...
    home = tmp_path / "home"
    repo = home / ".telegram-agent-bot" / "app" / "TelegramAgentBot"
    script = _prepare_repo(repo)
    env = _prepare_env(tmp_path, home)
    ...
    launcher = home / ".telegram-agent-bot" / "bin" / "telegram-agent-bot-launch"
