...
def test_load_update_settings_reads_env(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("TELEGRAM_AGENT_BOT_AUTO_UPDATE", "true")
...
def test_load_codex_update_settings_reads_env(monkeypatch) -> None:
    monkeypatch.setenv("TELEGRAM_AGENT_BOT_CODEX_UPDATE_CHECK", "true")
    monkeypatch.setenv("TELEGRAM_AGENT_BOT_CODEX_AUTO_UPDATE", "true")
    monkeypatch.setenv("TELEGRAM_AGENT_BOT_CODEX_UPDATE_PACKAGE", "@openai/codex")
    monkeypatch.setenv(
        "TELEGRAM_AGENT_BOT_CODEX_COMMAND", "IS_SANDBOX=1 codex --danger"
    )
    monkeypatch.setenv("TELEGRAM_AGENT_BOT_CODEX_UPDATE_NPM", "npm")
...
def test_dirty_worktree_skips_update(tmp_path: Path) -> None:
    (tmp_path / ".git").mkdir()
    runner = FakeRunner(
        {("git", "status", "--porcelain"): " M src/telegram_agent_bot/main.py\n"}
    )
