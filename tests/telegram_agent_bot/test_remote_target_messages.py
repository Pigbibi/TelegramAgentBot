...
async def test_handle_new_message_routes_remote_target_session() -> None:
    ...
    with (
        ...
        patch(
            "telegram_agent_bot.bot.build_response_parts",
...
@pytest.mark.asyncio
async def test_handle_new_message_hides_plain_reasoning() -> None:
    bot = AsyncMock()
    msg = NewMessage(
        session_id="local-1",
        text="The user just asked a question.",
        is_complete=True,
        content_type="thinking",
        source_offset=123,
    )
...
async def test_handle_new_message_reports_remote_usage_limit() -> None:
    ...
    with (
        patch("telegram_agent_bot.bot.session_manager") as mock_sm,
        patch("telegram_agent_bot.bot.safe_send", new_callable=AsyncMock) as safe_send,
    ):
        mock_sm.find_users_for_session = AsyncMock(return_value=[])
        mock_sm.find_users_for_target_session.return_value = [(12345, "", 42)]
        mock_sm.resolve_chat_id.return_value = -1001234567890
...
async def test_handle_new_message_reports_repeated_local_usage_limit() -> None:
    ...
    with (
        patch("telegram_agent_bot.bot.session_manager") as mock_sm,
        patch("telegram_agent_bot.bot.safe_send", new_callable=AsyncMock) as safe_send,
        patch("telegram_agent_bot.bot.get_next_account_name", return_value="backup"),
        patch(
            "telegram_agent_bot.bot.config",
