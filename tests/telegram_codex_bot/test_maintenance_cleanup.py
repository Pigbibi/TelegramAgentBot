...
from telegram_codex_bot import maintenance_cleanup as cleanup
...
def test_remove_path_refuses_protected_runtime_dir(tmp_path: Path) -> None:
    ...
    protected_file = config.home / ".telegram-codex-bot" / "state.json"
    protected_file.write_text("state")

    cleanup.remove_path(config.home / ".telegram-codex-bot", config, stats)
...
def test_clean_tmp_keeps_bot_locks_and_removes_old_artifacts(tmp_path: Path) -> None:
    ...
    lock_dir = config.tmp_path / "telegram-codex-bot-lock-active"
