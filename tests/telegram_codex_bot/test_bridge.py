...
from telegram_codex_bot.bridge import (
    BridgeConfig,
    BridgeTarget,
    GitHubIssue,
...
def test_load_config(tmp_path: Path) -> None:
    ...
    cfg_path.write_text(
        json.dumps(
            {
                "bridge_mode": "targets",
                "tmux_socket": "telegram-codex-bot",
                "issue_limit": 20,
                "body_limit": 100,
                "comment_limit": 2,
    ...
    assert cfg.tmux_socket == "telegram-codex-bot"
...
def test_process_target_retries_retryable_gh_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr("telegram_codex_bot.bridge.subprocess.run", fake_run)
    monkeypatch.setattr(
        "telegram_codex_bot.bridge.time.sleep", lambda *_args, **_kwargs: None
    )
    monkeypatch.setattr(
        "telegram_codex_bot.bridge.random.uniform", lambda *_args, **_kwargs: 0.0
    )
    monkeypatch.setattr(
        "telegram_codex_bot.bridge.fetch_issue",
        lambda repo, issue_number, **kwargs: issue,
    ...
    monkeypatch.setattr(
        "telegram_codex_bot.bridge.dispatch_to_tmux", lambda *args, **kwargs: None
    )
