"""Tests for telegram-codex-bot.utils: app_dir, atomic_write_json, read_cwd_from_jsonl."""
...
from telegram_codex_bot.utils import atomic_write_json, app_dir, read_cwd_from_jsonl


class TestAppDir:
    def test_returns_env_var_path(self, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setenv("TELEGRAM_CODEX_BOT_DIR", "/custom/config")
        assert app_dir() == Path("/custom/config")

    def test_returns_default_without_env(self, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.delenv("TELEGRAM_CODEX_BOT_DIR", raising=False)
        assert app_dir() == Path.home() / ".telegram-codex-bot"
