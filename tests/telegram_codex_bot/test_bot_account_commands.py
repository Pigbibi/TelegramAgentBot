...
def test_extract_device_login_details_strips_ansi() -> None:
    from telegram_codex_bot.bot import _extract_device_login_details
...
async def test_codex_account_list_reports_status() -> None:
    ...
    with (
        ...
        patch("telegram_codex_bot.bot.list_account_names", return_value=["backup"]),
        patch("telegram_codex_bot.bot.get_current_account_name", return_value=None),
        patch(
            "telegram_codex_bot.bot.safe_reply", new_callable=AsyncMock
        ) as safe_reply,
    ):
        from telegram_codex_bot.bot import codex_account_command
...
async def test_codex_account_use_selects_saved_account() -> None:
    ...
    with (
        patch("telegram_codex_bot.bot.is_user_allowed", return_value=True),
        patch("telegram_codex_bot.bot.list_account_names", return_value=["backup"]),
        patch("telegram_codex_bot.bot.remember_current_account") as remember,
        patch(
            "telegram_codex_bot.bot.safe_reply", new_callable=AsyncMock
        ) as safe_reply,
    ):
...
async def test_codex_login_command_rejects_invalid_account_name() -> None:
    ...
    with (
        ...
        from telegram_codex_bot.bot import codex_login_command
