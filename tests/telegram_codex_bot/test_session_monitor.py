...
class TestReadNewLinesOffsetRecovery:
    ...
    @pytest.mark.asyncio
    async def test_stale_backlog_before_latest_user_is_not_replayed(
        self, monitor, tmp_path, make_jsonl_entry
    ):
        """When monitor lags, do not replay old assistant replies before a newer prompt."""
        jsonl_file = tmp_path / "session.jsonl"
    ...
    async def test_rebinds_bound_window_with_stale_cwd(self, monitor, tmp_path):
        ...
        with (
            patch(
                "telegram_codex_bot.session_monitor.list_account_homes", return_value=[]
            ),
            patch("telegram_codex_bot.session_monitor.tmux_manager") as mock_tmux,
            patch("telegram_codex_bot.session.session_manager") as mock_sm,
        ):
    ...
    async def test_skips_external_transcript_auto_bind_when_account_homes_exist(
        ...
        """Do not bind unrelated ~/.codex history when telegram-codex-bot account homes exist."""
        ...
        with (
            patch(
                "telegram_codex_bot.session_monitor.list_account_homes",
