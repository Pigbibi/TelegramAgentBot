...
from telegram_codex_bot import hook as hook_module
from telegram_codex_bot.hook import (
    _ROLLOUT_SESSION_RE,
    _UUID_RE,
    _install_hook,
    _is_hook_installed,
                ...
                "SessionStart": [
                    {
                        "hooks": [
                            {
                                "type": "command",
                                "command": "/usr/bin/telegram-codex-bot hook",
                                "timeout": 5,
                            }
                        ]
                    }
...
class TestHookMainValidation:
    def _run_hook_main(
        ...
        monkeypatch.setattr(sys, "argv", ["telegram-codex-bot", "hook"])
    ...
    def test_valid_rollout_session_writes_session_map(
        ...
        monkeypatch.setattr(
            ...
            lambda *args, **kwargs: SimpleNamespace(
                stdout="telegram-codex-bot:@12:demo", returncode=0
            ),
        )
        self._run_hook_main(
            monkeypatch,
...
class TestInstallHook:
    ...
    def test_install_writes_config_and_hooks_json(
        ...
        monkeypatch.setattr(
            hook_module.shutil, "which", lambda _: "/usr/local/bin/telegram-codex-bot"
        )
                ...
                "SessionStart": [
                    {
                        ...
                        "hooks": [
                            {
                                "type": "command",
                                "command": "/usr/local/bin/telegram-codex-bot hook",
    ...
    def test_install_removes_legacy_ccbot_hook(
        self,
        monkeypatch: pytest.MonkeyPatch,
        tmp_path: Path,
        capsys: pytest.CaptureFixture[str],
                        ...
                        "SessionStart": [
                            {
                                ...
                                "hooks": [
                                    {
                                        "type": "command",
                                        "command": "/old/venv/bin/ccbot hook",
    ...
    def test_install_preserves_existing_hooks(
        ...
        assert (
            ...
            == "/opt/bin/telegram-codex-bot hook"
        )
