...
Handles two execution modes:
  1. `telegram-agent-bot hook` — delegates to hook.hook_main() for Codex hook processing.
  2. `telegram-agent-bot update` — checks and fast-forwards source-checkout installs.
...
class AlreadyRunningError(RuntimeError):
    """Raised when another local telegram-agent-bot instance already holds the lock."""


_INSTANCE_LOCK_HANDLE = None
_USAGE = """Usage:
  telegram-agent-bot               Start the Telegram bot
  telegram-agent-bot hook [args]   Process or install the Codex hook
...
def acquire_instance_lock(lock_path: Path):
    ...
    try:
        ...
        raise AlreadyRunningError(
            f"telegram-agent-bot is already running: {lock_path}"
        ) from exc
...
def main() -> None:
    ...
    if mode == "version":
        ...
        print(f"telegram-agent-bot {__version__}")
        return
    if mode == "exit":
        return
    ...
    try:
        _INSTANCE_LOCK_HANDLE = acquire_instance_lock(
            config.config_dir / "telegram_agent_bot.lock"
        )
        atexit.register(_release_instance_lock)
    except AlreadyRunningError:
        logger.error(
            "Another local telegram-agent-bot instance is already running; exiting."
        )
        return
