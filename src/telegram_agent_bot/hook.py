...
def _find_cli_path() -> str:
    """Find the full path to the telegram-agent-bot executable.

    Priority:
    1. shutil.which("telegram-agent-bot") - if telegram-agent-bot is in PATH
    2. Same directory as the Python interpreter (for venv installs)
    """
    # Try PATH first
    cli_path = shutil.which("telegram-agent-bot")
    if cli_path:
        return cli_path

    # Fall back to the directory containing the Python interpreter
    # This handles the case where telegram-agent-bot is installed in a venv
    python_dir = Path(sys.executable).parent
    cli_in_venv = python_dir / "telegram-agent-bot"
    if cli_in_venv.exists():
        return str(cli_in_venv)

    # Last resort: assume it will be in PATH
    return "telegram-agent-bot"


def _is_telegram_agent_bot_hook_command(command: str) -> bool:
    """Return True if command invokes ``telegram-agent-bot hook``."""
    try:
    ...
    for entry in session_start:
        ...
        for h in inner_hooks:
            if not isinstance(h, dict):
                continue
            cmd = h.get("command", "")
            if isinstance(cmd, str) and _is_telegram_agent_bot_hook_command(cmd):
                return h
    return None


def _is_hook_installed(settings: dict) -> bool:
    """Check if telegram-agent-bot hook is already installed in hooks.json."""
