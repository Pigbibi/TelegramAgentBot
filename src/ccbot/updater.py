"""Self-update helpers for source-checkout ccbot installs.
...
class UpdateSettings:
    """Runtime settings for ccbot self-update checks."""
...
def load_update_settings() -> UpdateSettings:
    ...
    state_path = os.getenv("CCBOT_UPDATE_STATE_FILE")
    uv_executable = os.getenv("CCBOT_UPDATE_UV") or shutil.which("uv")
    return UpdateSettings(
        enabled=_parse_bool(os.getenv("CCBOT_AUTO_UPDATE"), default=False),
        require_idle=_parse_bool(os.getenv("CCBOT_UPDATE_REQUIRE_IDLE"), default=True),
        interval_seconds=max(
            0,
            _parse_int(
                os.getenv("CCBOT_UPDATE_INTERVAL_SECONDS"),
        ...
        busy_retry_seconds=max(
            60,
            _parse_int(
                os.getenv("CCBOT_UPDATE_BUSY_RETRY_SECONDS"),
                default=DEFAULT_BUSY_RETRY_SECONDS,
            ),
        ),
        remote=(os.getenv("CCBOT_UPDATE_REMOTE") or "").strip() or None,
        branch=(os.getenv("CCBOT_UPDATE_BRANCH") or "").strip() or None,
        state_file=Path(state_path).expanduser()
        if state_path
        else config_dir / "update_state.json",
        run_uv_sync=_parse_bool(os.getenv("CCBOT_UPDATE_RUN_UV_SYNC"), default=True),
...
def load_codex_update_settings() -> CodexUpdateSettings:
    ...
    codex_command = os.getenv("CCBOT_CODEX_COMMAND", "codex")
    codex_executable = _extract_executable(codex_command) or "codex"
    return CodexUpdateSettings(
        enabled=_parse_bool(os.getenv("CCBOT_CODEX_UPDATE_CHECK"), default=False),
        auto_update=_parse_bool(os.getenv("CCBOT_CODEX_AUTO_UPDATE"), default=False),
        package=(os.getenv("CCBOT_CODEX_UPDATE_PACKAGE") or DEFAULT_CODEX_PACKAGE),
        codex_executable=shutil.which(codex_executable) or codex_executable,
        npm_executable=os.getenv("CCBOT_CODEX_UPDATE_NPM") or shutil.which("npm"),
    )
    ...
    *,
    ...
    if repo is None or not (repo / ".git").exists():
        return UpdateResult(
            ...
            message=(
                "Self-update is only supported when ccbot runs from a git checkout."
            ),
        )
    ...
    try:
        ...
        if behind <= 0:
            result = UpdateResult(
                ...
                message=f"ccbot is already up to date with {ref}.",
            )
            _write_update_state(settings.state_file, timestamp, result)
            return result
        ...
        result = UpdateResult(
            ...
            message=f"Updated ccbot from {old_revision or 'unknown'} to {new_revision or 'unknown'}.",
        )
        _write_update_state(settings.state_file, timestamp, result)
        return result
    except (OSError, subprocess.SubprocessError, UpdateError) as exc:
...
def restart_current_process(argv: Sequence[str] | None = None) -> None:
    ...
    if not command:
        command = [sys.executable, "-m", "ccbot.main"]
...
def maybe_auto_update(argv: Sequence[str] | None = None) -> UpdateResult | None:
    ...
    if settings.require_idle:
        logger.info("Startup auto-update deferred until ccbot is idle")
        return None
    ...
    if result.updated and result.restart_required:
        logger.warning("ccbot updated; restarting current process")
...
async def auto_update_loop(argv: Sequence[str] | None = None) -> None:
    ...
    while True:
        ...
        if settings.require_idle:
            ...
            if blockers:
                logger.info(
                    "Auto-update deferred; ccbot is not idle: %s",
                    "; ".join(blockers[:5]),
                )
...
def _print_update_usage() -> None:
    print(
        "Usage:\n"
        "  ccbot update           Fast-forward the git checkout if an update exists\n"
        "  ccbot update --check   Check only; do not change files\n"
        "  ccbot update --help    Show this help message"
    )
...
def _print_codex_update_usage() -> None:
    print(
        "Usage:\n"
        "  ccbot codex-update           Update global npm Codex CLI if needed\n"
        "  ccbot codex-update --check   Check only; do not change files\n"
        "  ccbot codex-update --help    Show this help message"
    )
...
def update_main(argv: Sequence[str]) -> int:
    ...
    if result.updated:
        print("Restart ccbot to use the updated code.")
