...
.env loading priority: local .env (cwd) > $TELEGRAM_CODEX_BOT_DIR/.env (default ~/.telegram-codex-bot).
...
def _parse_project_roots(raw: str, fallback: Path) -> list[ProjectRoot]:
    """Parse TELEGRAM_CODEX_BOT_PROJECT_ROOTS as label=path pairs."""
...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.tmux_session_name = os.getenv(
            "TELEGRAM_CODEX_BOT_TMUX_SESSION_NAME", "telegram-codex-bot"
        )
        ...
        default_projects_path = os.getenv("TELEGRAM_CODEX_BOT_DEFAULT_PROJECTS_PATH")
        self.default_projects_path = (
            Path(default_projects_path).expanduser()
            if default_projects_path
            else Path.home() / "Projects"
        )
        project_roots = os.getenv("TELEGRAM_CODEX_BOT_PROJECT_ROOTS", "")
        ...
        self.monitor_poll_interval = float(
            os.getenv("TELEGRAM_CODEX_BOT_MONITOR_POLL_INTERVAL", "2.0")
        )
        ...
        self.show_user_messages = (
            os.getenv("TELEGRAM_CODEX_BOT_SHOW_USER_MESSAGES", "true").lower()
            != "false"
        )
        ...
        self.show_commentary_messages = (
            os.getenv("TELEGRAM_CODEX_BOT_SHOW_COMMENTARY_MESSAGES", "false").lower()
            == "true"
        )
        ...
        self.show_external_resume_sessions = (
            os.getenv("TELEGRAM_CODEX_BOT_SHOW_EXTERNAL_RESUME_SESSIONS", "").lower()
            == "true"
        )
