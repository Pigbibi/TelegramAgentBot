...
Responsibilities:
  - Persist/load state to ~/.telegram-codex-bot/state.json.
...
class SessionManager:
    ...
    async def _cleanup_stale_session_map_entries(self, live_ids: set[str]) -> None:
        ...
        When windows are closed externally (outside telegram-codex-bot), session_map.json
    ...
    async def load_session_map(self) -> None:
        ...
        Keys in session_map are formatted as "tmux_session:window_id" (e.g. "telegram-codex-bot:@12").
    ...
    def _is_account_home_transcript(file_path: Path) -> bool:
        """Return whether a transcript lives under a telegram-codex-bot per-account CODEX_HOME."""
        try:
    ...
    def _is_external_resume_transcript(self, file_path: Path) -> bool:
        """Return whether a transcript comes from the user's non-telegram-codex-bot Codex history."""
