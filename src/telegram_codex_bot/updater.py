"""Self-update helpers for source-checkout telegram-codex-bot installs.
...
class UpdateSettings:
    """Runtime settings for telegram-codex-bot self-update checks."""
...
def load_update_settings() -> UpdateSettings:
    ...
    config_dir = app_dir()
    state_path = os.getenv("TELEGRAM_CODEX_BOT_UPDATE_STATE_FILE")
    uv_executable = os.getenv("TELEGRAM_CODEX_BOT_UPDATE_UV") or shutil.which("uv")
    return UpdateSettings(
        enabled=_parse_bool(os.getenv("TELEGRAM_CODEX_BOT_AUTO_UPDATE"), default=False),
        require_idle=_parse_bool(
            os.getenv("TELEGRAM_CODEX_BOT_UPDATE_REQUIRE_IDLE"), default=True
        ),
        interval_seconds=max(
            0,
            _parse_int(
                os.getenv("TELEGRAM_CODEX_BOT_UPDATE_INTERVAL_SECONDS"),
        ...
        busy_retry_seconds=max(
            60,
            _parse_int(
                os.getenv("TELEGRAM_CODEX_BOT_UPDATE_BUSY_RETRY_SECONDS"),
                default=DEFAULT_BUSY_RETRY_SECONDS,
            ),
        ),
        remote=(os.getenv("TELEGRAM_CODEX_BOT_UPDATE_REMOTE") or "").strip() or None,
        branch=(os.getenv("TELEGRAM_CODEX_BOT_UPDATE_BRANCH") or "").strip() or None,
        state_file=Path(state_path).expanduser()
        if state_path
        else config_dir / "update_state.json",
        run_uv_sync=_parse_bool(
            os.getenv("TELEGRAM_CODEX_BOT_UPDATE_RUN_UV_SYNC"), default=True
        ),
...
def load_codex_update_settings() -> CodexUpdateSettings:
    ...
    return CodexUpdateSettings(
        ...
        auto_update=_parse_bool(
            os.getenv("TELEGRAM_CODEX_BOT_CODEX_AUTO_UPDATE"), default=False
        ),
        package=(
    ...
    *,
    ...
    if repo is None or not (repo / ".git").exists():
        return UpdateResult(
            ...
            message=(
                "Self-update is only supported when telegram-codex-bot runs from a git checkout."
            ),
        )
    ...
    try:
        ...
        if behind <= 0:
            result = UpdateResult(
                ...
                message=f"telegram-codex-bot is already up to date with {ref}.",
            )
            _write_update_state(settings.state_file, timestamp, result)
            return result
        ...
        result = UpdateResult(
            ...
            message=f"Updated telegram-codex-bot from {old_revision or 'unknown'} to {new_revision or 'unknown'}.",
        )
        _write_update_state(settings.state_file, timestamp, result)
        return result
    except (OSError, subprocess.SubprocessError, UpdateError) as exc:
...
def restart_current_process(argv: Sequence[str] | None = None) -> None:
    ...
    if not command:
        command = [sys.executable, "-m", "telegram_codex_bot.main"]
...
def maybe_auto_update(argv: Sequence[str] | None = None) -> UpdateResult | None:
    ...
    if settings.require_idle:
        logger.info("Startup auto-update deferred until telegram-codex-bot is idle")
        return None
    ...
    if result.updated and result.restart_required:
        logger.warning("telegram-codex-bot updated; restarting current process")
    ...
    *,
    ...
    while True:
        ...
        if settings.require_idle:
            ...
            if blockers:
                logger.info(
                    "Auto-update deferred; telegram-codex-bot is not idle: %s",
                    "; ".join(blockers[:5]),
                )
...
def _print_update_usage() -> None:
    print(
        "Usage:\n"
        "  telegram-codex-bot update           Fast-forward the git checkout if an update exists\n"
        "  telegram-codex-bot update --check   Check only; do not change files\n"
        "  telegram-codex-bot update --help    Show this help message"
    )
...
def update_main(argv: Sequence[str]) -> int:
    ...
    if result.updated:
        print("Restart telegram-codex-bot to use the updated code.")
