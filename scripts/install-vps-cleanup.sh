...
SERVICE_NAME="${TELEGRAM_CODEX_CLEANUP_SYSTEMD_SERVICE_NAME:-io.github.telegramcodexbot.cleanup.service}"
TIMER_NAME="${SERVICE_NAME%.service}.timer"
SERVICE_PATH="${SYSTEMD_DIR}/${SERVICE_NAME}"
TIMER_PATH="${SYSTEMD_DIR}/${TIMER_NAME}"
LAUNCHER_PATH="${BIN_DIR}/telegram-codex-cleanup"
PATH_VALUE="/usr/local/bin:/usr/bin:/bin:${HOME}/.local/bin"

CLEANUP_ON_CALENDAR="${TELEGRAM_CODEX_CLEANUP_ON_CALENDAR:-*-*-* 04:20:00}"
CLEANUP_RANDOMIZED_DELAY="${TELEGRAM_CODEX_CLEANUP_RANDOMIZED_DELAY:-30m}"
CLEANUP_MAX_USED_PERCENT="${TELEGRAM_CODEX_CLEANUP_MAX_USED_PERCENT:-80}"
CLEANUP_MIN_FREE_GB="${TELEGRAM_CODEX_CLEANUP_MIN_FREE_GB:-6}"
CLEANUP_TMP_RETENTION_DAYS="${TELEGRAM_CODEX_CLEANUP_TMP_RETENTION_DAYS:-2}"
CLEANUP_CODEX_SESSION_RETENTION_DAYS="${TELEGRAM_CODEX_CLEANUP_CODEX_SESSION_RETENTION_DAYS:-30}"
CLEANUP_RUNNER_DIAG_RETENTION_DAYS="${TELEGRAM_CODEX_CLEANUP_RUNNER_DIAG_RETENTION_DAYS:-7}"
...
export HOME="$HOME"
export TELEGRAM_CODEX_BOT_DIR="$TELEGRAM_CODEX_BOT_DIR"
cd "$REPO_DIR"
exec /usr/bin/env uv run python -m telegram_codex_bot.maintenance_cleanup \\
  --yes \\
